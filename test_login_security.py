...
def test_username_validation(username: str, description: str):
    ...
    try:
        ...
        if result["allowed"]:
            print(f"РАЗРЕШЕН (слой: {result.get('layer', 'unknown')})")
            print(f"ВНИМАНИЕ: Этот username может быть опасным!")
            return False
        else:
...
def test_password_validation(password: str, description: str):
    ...
    print("Password не попадает в SQL запрос (используется bcrypt), поэтому безопасен")
    return True
...
def main():
    ...
    print(f"Разрешено (требует внимания): {allowed_count}")
    print(f"Ошибок: {error_count}")
    
    if allowed_count > 0:
        print("\nВНИМАНИЕ: Некоторые вредоносные username были разрешены!")
    else:
